    ...
    return (
        <div className="absolute right-0 top-full z-[100] w-56 pt-2">
            <div className="overflow-hidden rounded-lg border border-dll-border bg-dll-surface p-2 shadow-lg">
                ...
                {user &&
                    <>
                        <Link href={`/dashboard/${user.role === "admin" ? "admin" : "user"}/profile`} className="block rounded-md px-4 py-2 text-sm text-dll-text hover:bg-dll-surface-alt hover:text-dll-primary">
                            My Profile
                        </Link>
                        <Link href={`/dashboard/${user.role === "admin" ? "admin" : "user"}`} className="block rounded-md px-4 py-2 text-sm text-dll-text hover:bg-dll-surface-alt hover:text-dll-primary">
                            Dashboard
                        </Link>
